...
N = min([length(t_real), length(uR_real), length(uT_real), length(uN_real)]);
t_real  = t_real(1:N);
uR_real = uR_real(1:N);
uT_real = uT_real(1:N);
uN_real = uN_real(1:N);

u_vec = [uR_real, uT_real, uN_real];

dV_R = trapz(t_real, abs(uR_real));
dV_T = trapz(t_real, abs(uT_real));
dV_N = trapz(t_real, abs(uN_real));
dV_total = trapz(t_real, vecnorm(u_vec,2,2));
...
fprintf('DeltaV radial         = %.4f m/s\n', dV_R);
fprintf('DeltaV tangential     = %.4f m/s\n', dV_T);
fprintf('DeltaV normal         = %.4f m/s\n', dV_N);
fprintf('DeltaV total          = %.4f m/s\n', dV_total);
