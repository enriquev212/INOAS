function [u, delta_Ulast, slack_opt] = MPC_INOAS(x_estim, covariance_estim, t_sim, varargin)
    ...
    %% Constraints
    [A,b] = MPCLinearConstraints(Umin, Umax, Ymin, Ymax, deltaUmax, ...
    ...
    U_check = E*u + H*delta_U_check;

    if ~cfg.quiet && mod(round(t_sim),10) == 0
        fprintf('t=%.1f | exit=%d | viol=%.1e | maxU=%.4f | err=%.3f m\n', ...
            t_sim, exitflag, viol, max(abs(U_check)), norm(x_rel_estim(1:3)));
    end
    %% Apply first control
    delta_u = delta_U(1:m);
    
    u_ref = u_current + delta_u;
